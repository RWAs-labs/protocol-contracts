    ...
    INotSupportedMethods
{
    ...
    /// @notice The address of the MuseConnector contract.
    address public museConnector;
    /// @notice The address of the Muse token contract.
    ...
    /// @notice Initialize with tss address. address of muse token and admin account set as DEFAULT_ADMIN_ROLE.
    /// @dev Using admin to authorize upgrades and pause, and tss for tss role.
    function initialize(address tssAddress_, address museToken_, address admin_) public initializer {
        if (tssAddress_ == address(0) || museToken_ == address(0)) {
            revert ZeroAddress();
        }
    ...
    /// @param museConnector_ Address of the connector contract.
    function setConnector(address museConnector_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (museConnector_ == address(0)) revert ZeroAddress();
        if (museConnector != address(0)) revert ConnectorInitialized();

        _grantRole(ASSET_HANDLER_ROLE, museConnector_);
        museConnector = museConnector_;
    }
    ...
    function _transferFromToAssetHandler(address from, address token, uint256 amount) private {
        if (token == museToken) {
            ...
            // if (!IERC20(token).approve(museConnector, amount)) revert ApprovalFailed();
            // // send tokens to connector
            // MuseConnectorBase.sol(museConnector).receiveTokens(amount);
        } else {
            // transfer to custody
    ...
    function _transferToAssetHandler(address token, uint256 amount) private {
        if (token == museToken) {
            // transfer to connector
            // approve connector to handle tokens depending on connector version (eg. lock or burn)
            if (!IERC20(token).approve(museConnector, amount)) revert ApprovalFailed(token, museConnector);
            // send tokens to connector
            MuseConnectorBase(museConnector).receiveTokens(amount);
        } else {
            // transfer to custody
            if (!IERC20Custody(custody).whitelisted(token)) revert NotWhitelistedInCustody(token);
