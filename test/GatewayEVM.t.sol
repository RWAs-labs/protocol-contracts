...
contract GatewayEVMTest is Test, IGatewayEVMErrors, IGatewayEVMEvents, IReceiverEVMEvents, IERC20CustodyEvents {
    ...
    function setUp() public {
        ...
        muse = new MuseNonEth(tssAddress, tssAddress);
        address proxy = Upgrades.deployUUPSProxy(
            "GatewayEVM.sol", abi.encodeCall(GatewayEVM.initialize, (tssAddress, address(muse), owner))
        );
        ...
        proxy = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(MuseConnectorNonNative.initialize, (address(gateway), address(muse), tssAddress, owner))
        );
        museConnector = MuseConnectorNonNative(proxy);

        vm.prank(tssAddress);
        muse.updateTssAndConnectorAddresses(tssAddress, address(museConnector));
    ...
    INotSupportedMethods
{
    ...
    function setUp() public {
        ...
        proxy = Upgrades.deployUUPSProxy(
            "MuseConnectorNonNative.sol",
            abi.encodeCall(MuseConnectorBase.initialize, (address(gateway), address(muse), tssAddress, owner))
        );
        museConnector = MuseConnectorNonNative(proxy);
        ...
        vm.prank(tssAddress);
        museConnector.withdraw(owner, ownerAmount, "");
    ...
    function testDepositMuseToConnector() public {
        uint256 amount = 100_000;
        muse.approve(address(gateway), amount);
        ...
        emit Deposited(owner, destination, amount, address(muse), "", revertOptions);
        gateway.deposit(destination, amount, address(muse), revertOptions);

        // uint256 ownerAmountAfter = muse.balanceOf(owner);
        // assertEq(ownerAmount - amount, ownerAmountAfter);
    }
