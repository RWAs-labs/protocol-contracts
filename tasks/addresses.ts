...
import { getEndpoints } from "@rwas-labs/networks";
...
import muse_mainnet_addresses from "./addresses.mainnet.json";
import muse_testnet_addresses from "./addresses.testnet.json";

declare const hre: any;

type Network = "muse_mainnet" | "muse_testnet";
...
const api = {
  muse_mainnet: {
    evm: "https://musechain-evm.blockpi.network/v1/rpc/public",
    rpc: "https://musechain.blockpi.network/lcd/v1/public",
  },
  muse_testnet: {
    evm: "https://musechain-athens-evm.blockpi.network/v1/rpc/public",
    rpc: "https://musechain-athens.blockpi.network/lcd/v1/public",
  },
};

const fetchChains = async (network: Network) => {
  const URL = `${api[network].rpc}/muse-chain/observer/supportedChains`;
  try {
...
const fetchTssData = async (chains: any, addresses: any, network: Network) => {
  const isMainnet = network == "muse_mainnet";
  const bitcoinChainID = isMainnet ? "8332" : "18332";
  const URL = `${api[network].rpc}/muse-chain/observer/get_tss_address/${bitcoinChainID}`;
  try {
...
const fetchSystemContract = async (addresses: any, network: Network) => {
  const chain_id = network === "muse_mainnet" ? 7000 : 7001;
  const URL = `${api[network].rpc}/muse-chain/fungible/system_contract`;
  try {
    ...
    if (systemContractResponse.status === 200) {
      ...
      addresses.push({
        address: systemContractResponse.data.SystemContract.connector_mevm,
        category: "messaging",
        chain_id,
        chain_name: network,
        type: "connector",
...
const fetchForeignCoinsData = async (chains: any, addresses: any, network: Network) => {
  const chain_id = network === "muse_mainnet" ? 7000 : 7001;
  const URL = `${api[network].rpc}/muse-chain/fungible/foreign_coins`;
  try {
    ...
    if (foreignCoinsResponse.status === 200) {
      foreignCoinsResponse.data.foreignCoins.forEach((token: any) => {
        addresses.push({
          address: token.mrc20_contract_address,
          asset: token.asset,
          category: "omnichain",
          chain_id,
          chain_name: network,
...
const fetchAthensAddresses = async (addresses: any, network: Network) => {
  ...
  try {
    ...
    addresses.push({ ...common, address: await sc.wMuseContractAddress(), type: "museToken" });
    addresses.push({ ...common, address: await sc.uniswapv2Router02Address(), type: "uniswapV2Router02" });
    // addresses.push({ ...common, address: await sc.museConnectorMEVMAddress(), type: "museConnectorMEVM" });
    addresses.push({ ...common, address: await sc.FUNGIBLE_MODULE_ADDRESS(), type: "fungibleModule" });
  } catch (error) {
    console.error("Error fetching addresses from MuseChain:", error);
  }
};
...
const fetchChainSpecificAddresses = async (chains: any, addresses: any, network: Network) => {
  await Promise.all(
    chains.map(async (chain: any) => {
      return axios
        .get(`${api[network].rpc}/muse-chain/observer/get_chain_params_for_chain/${chain.chain_id}`)
        .then(({ data }) => {
          const museToken = data.chain_params.muse_token_contract_address;
          if (museToken && museToken != ethers.ZeroAddress) {
            addresses.push({
              address: museToken,
              category: "messaging",
...
const fetchFactoryV2 = async (addresses: any, network: Network) => {
  ...
  for (const router of routers) {
    ...
    try {
      ...
      // Skip MuseChain as we've already added MUSE token
...
const main = async (args: any, hre: HardhatRuntimeEnvironment) => {
  ...
  if (n === "muse_testnet") {
    addresses.push(...muse_testnet_addresses);
  } else if (n === "muse_mainnet") {
    addresses.push(...muse_mainnet_addresses);
  } else {
    throw new Error(`Unsupported network: ${n}. Must be 'muse_testnet' or 'muse_mainnet'.`);
  }
